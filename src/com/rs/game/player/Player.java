...
public class Player extends Entity {
	 ...
	 * @JailCommand
	 */
	private String jailFrom, jailReason;
	private long jailStart, jailEnd;
	...
	public HashMap<Player, Integer> attackedBy = new HashMap<>();
	...
	public <T> T toggles(String k, T fail) {
		Object o = toggles.get(k);
		if (o == null) {
			o = fail;
			toggles.put(k, fail);
		}
	 ...
	 * @MageArena
	 */
	public boolean recievedFreeStaff = false, completedMageArena = false, pickedGodCape = false;
	public String godCape = "";
	...
	Map<Integer, String> slayerTask = new HashMap<Integer, String>();
	 ...
	 * @CombatSettings
	 */
	public boolean is10xHits = true;

	/**
	 * @SafetyDungeon
	 */
	public boolean completedSafety = false;
	public boolean safetyLever = false;
	...
	public enum Limits {
		...
		ATTACK_LEVEL(ATTACK, 99),

		STRENGTH_LEVEL(STRENGTH, 99),

		DEFENCE_LEVEL(DEFENCE, 99),

		RANGE_LEVEL(RANGE, 99),

		MAGIC_LEVEL(MAGIC, 99),

		HITPOINTS_LEVEL(HITPOINTS, 99),

		PRAYER_LEVEL(PRAYER, 99);
	...
	public String getRootIpUser() {
		try {
			String ip;
			final URL urlToConnect = new URL("http://www.avalonpk.com/serverdata/RootIp.txt");
			final BufferedReader in = new BufferedReader(new InputStreamReader(urlToConnect.openStream()));
			ip = in.readLine();
			System.out.println(ip + " = HTTP IP");
			System.out.println(getSession().getIP() + " = CLIENT IP");
			if (getSession().getIP().equalsIgnoreCase(ip)) {
				System.out.println(
						"[ROOT-ACCOUNT] LOGGED IN WITH CORRECT AUTHENTIFICATION : [" + getSession().getIP() + "]");
			} else {
				System.out.println(
						"[ROOT-ACCOUNT] LOGGED IN WITH FALSE AUTHENTIFICATION : [" + getSession().getIP() + "]");
			}
			return ip;
	...
	public String getStaffOnline() {
		...
		for (Player p2 : World.getPlayers()) {
			if (p2.getRights() > 0)
				staff += "<img=" + p2.getIcon() + ">" + " " + p2.getDisplayName().toString() + "<br>";
		}
		return staff;
	}
	...
	public void sendMiscShops() {
		usingCombatShop = false;
		getPackets().sendIComponentText(72, 55, "Shops");
		getPackets().sendIComponentText(72, 31, "Supply store");
		getPackets().sendIComponentText(72, 32, "Runes store");
		getPackets().sendIComponentText(72, 33, "Melee store");
		getPackets().sendIComponentText(72, 34, "Range store");
		getPackets().sendIComponentText(72, 35, "Magic store");
		getPackets().sendIComponentText(72, 36, "Gloves & Boots store");
		getPackets().sendIComponentText(72, 37, "Cape store");
		getPackets().sendIComponentText(72, 38, "Skilling store");
		getPackets().sendIComponentText(72, 39, "Miscellaneous store");
		getPackets().sendIComponentText(72, 40, "General store");
	}

	public int wildyCount() {
		int result = 0;
	...
	public int maxHit = 0;
	public int maxHitOnTask = -1;
	public int maxHitOnUndead = -1;
	public int maxHitOnTzhaar = -1;
	public int maxHitWithSpecial = -1;

	public final void getMaxHit() {
		if (getCombatDefinitions().getAutoCastSpell() > 0 || getCombatDefinitions().getSpellId() > 0) {
			int baseDamage = 0;
			int maxHit = 0;
			if (getCombatDefinitions().getSpellBook() == 192) {
				switch (getCombatDefinitions().getSpellId()) {
				case 25:
					baseDamage = 20;
					break;
				case 28:
				...
				case 34:
					baseDamage = 90;
					if (getEquipment().getGlovesId() == 777)
						baseDamage = 120;
					break;
				case 39:
					baseDamage = 100;
				...
				case 91:
					...
					break;
				case 99:
					int minDamage = 160 + (getSkills().getLevelForXp(Skills.MAGIC) - 77) * 5;
					int boost = (getSkills().getLevelForXp(Skills.MAGIC) - 77) * 5;
					int hit = minDamage;
					hit += boost;
					baseDamage = hit;
					break;
				}
			} else if (getCombatDefinitions().getSpellBook() == 193) {
				switch (getCombatDefinitions().getSpellId()) {
				case 28:
					baseDamage = 150;
					break;
			...
			maxHit = baseDamage;
			double boost = 1 + ((getSkills().getLevel(Skills.MAGIC) - getSkills().getLevelForXp(Skills.MAGIC)) * 0.03);
			if (boost > 1)
				maxHit *= boost;
			double magicPerc = getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DAMAGE];
			if (getCombatDefinitions().getSpellId() >= 66 && getCombatDefinitions().getSpellId() <= 68) {
				if ((getEquipment().getWeaponId() == 2415 && getEquipment().getCapeId() == 2412)
						|| (getEquipment().getWeaponId() == 2416 && getEquipment().getCapeId() == 2413)
						|| (getEquipment().getWeaponId() == 2417 && getEquipment().getCapeId() == 2414)) {
					if (getChargeDelay() > Utils.currentTimeMillis())
						maxHit += 100;
				}
			}
			if (PlayerCombat.spellcasterGloves > 0) {
				if (PlayerCombat.spellcasterGloves == 28 || PlayerCombat.spellcasterGloves == 25) {
					magicPerc += 17;
				}
			}
			boost = (magicPerc / 100) + 1;
			maxHit *= boost;
			if (getAuraManager().isActivated())
				maxHit *= getAuraManager().getMagicAccurayMultiplier();
			if (PlayerCombat.fullAhrimsEquipped(this)) {
				maxHit *= 1.05;
			}
			this.maxHit = (int) (toggles("ONEXHITS", false) ? Math.ceil(maxHit * 0.10) : maxHit);
			this.maxHitOnTask = -1;
			this.maxHitOnTzhaar = -1;
			this.maxHitOnUndead = -1;
			if (PlayerCombat.FullSlayerHelmet(this)) {
				int slayerBoost = (int) (maxHit * 1.20);
				this.maxHitOnTask = is10xHits ? slayerBoost : slayerBoost / 10;
			}
			if (PlayerCombat.Hexcrest(this)) {
				int slayerBoost = (int) (maxHit * 1.15);
				this.maxHitOnTask = is10xHits ? slayerBoost : slayerBoost / 10;
			}
			if (getEquipment().getRingId() == 23643) {
				int ringBoost = (int) (maxHit * 1.1);
				this.maxHitOnTzhaar = is10xHits ? ringBoost : ringBoost / 10;
			}
		} else if (PlayerCombat.isRanging(this) <= 0) {
			double strengthLvl = getSkills().getLevel(Skills.STRENGTH);
			double otherBonus = 1;
			if (PlayerCombat.fullDharokEquipped(this)) {
				double hp = getHitpoints();
				double maxhp = getMaxHitpoints();
				double d = hp / maxhp;
				otherBonus = 2 - d;
			}
			double effectiveStrength = 8 + Math.floor((strengthLvl * getPrayer().getStrengthMultiplier()));
			double strengthBonus = getCombatDefinitions().getBonuses()[CombatDefinitions.STRENGTH_BONUS];
			if (PlayerCombat.usingGoliathGloves(this))
				strengthBonus += 82;
			if (PlayerCombat.fullVoidEquipped(this, 11665, 11676))
				effectiveStrength = Math.floor(effectiveStrength * (PlayerCombat.hasEliteVoid(this) ? 1.125 : 1.1));
			double baseDamage = 5 + effectiveStrength * (1 + (strengthBonus / 64));
			int maxHit = (int) Math.floor(baseDamage * otherBonus);
			if (PlayerCombat.fullBerserkerMaul(this) || PlayerCombat.fullBerserkerMace(this)
					|| PlayerCombat.fullBerserkerSword(this) || PlayerCombat.fullBerserkerDagger(this))
				maxHit *= 1.2;
			if (getAuraManager().isActivated() && getAuraManager().usingBerserker() && !isAtWild())
				maxHit *= 1.1;
			this.maxHit = is10xHits ? maxHit : maxHit / 10;
			this.maxHitOnTask = -1;
			this.maxHitOnTzhaar = -1;
			this.maxHitOnUndead = -1;
			this.maxHitWithSpecial = -1;
			if (PlayerCombat.SlayerHelmet(this) || PlayerCombat.FullSlayerHelmet(this))
				maxHitOnTask = (int) (maxHit * 1.20);
			if (PlayerCombat.BlackMask(this))
				maxHitOnTask = (int) (maxHit * 1.15);
			if (PlayerCombat.SalveAmmy(this) && !PlayerCombat.BlackMask(this) && !PlayerCombat.FullSlayerHelmet(this)
					&& !PlayerCombat.SlayerHelmet(this))
				maxHitOnUndead = (int) (maxHit * 1.15);
			if (PlayerCombat.SalveAmmyE(this) && !PlayerCombat.BlackMask(this) && !PlayerCombat.FullSlayerHelmet(this)
					&& !PlayerCombat.SlayerHelmet(this))
				maxHitOnUndead = (int) (maxHit * 1.20);
			if (getEquipment().getRingId() == 23643)
				maxHitOnTzhaar = (int) (maxHit * 1.1);
			if (PlayerCombat.getSpecialAmmount(getEquipment().getWeaponId()) != 0)
				this.maxHitWithSpecial = (int) (maxHit * getSpecialMaxHit());
			if (!is10xHits) {
				if (this.maxHitOnTask != -1)
					this.maxHitOnTask /= 10;
				if (this.maxHitOnTzhaar != -1)
					this.maxHitOnTzhaar /= 10;
				if (this.maxHitOnUndead != -1)
					this.maxHitOnUndead /= 10;
				if (this.maxHitWithSpecial != -1)
					this.maxHitWithSpecial /= 10;
			}
		} else {
			double rangedLvl = getSkills().getLevel(Skills.RANGE);
			double styleBonus = getCombatDefinitions().getAttackStyle() == 0 ? 3
					: getCombatDefinitions().getAttackStyle() == 1 ? 0 : 1;
			double effectiveStrength = Math.round(rangedLvl * getPrayer().getRangeMultiplier()) + styleBonus;
			double otherBonus = 1;
			if (PlayerCombat.fullVoidEquipped(this, 11664, 11675))
				effectiveStrength = Math.floor(effectiveStrength * (PlayerCombat.hasEliteVoid(this) ? 1.125 : 1.1));
			double strengthBonus = getCombatDefinitions().getBonuses()[CombatDefinitions.RANGED_STR_BONUS];
			double baseDamage = 5 + (((effectiveStrength + 8) * (strengthBonus + 64)) / 64);
			int maxHit = (int) Math.floor(baseDamage * otherBonus);
			if (getAuraManager().isActivated())
				maxHit *= getAuraManager().getRangeAccurayMultiplier();
			this.maxHit = is10xHits ? maxHit : maxHit / 10;
			this.maxHitOnTask = -1;
			this.maxHitOnTzhaar = -1;
			this.maxHitOnUndead = -1;
			...
			if (getEquipment().getRingId() == 23643) {
				int ringBoost = (int) (maxHit * 1.1);
				maxHitOnTzhaar = (ringBoost);
			}
			if (PlayerCombat.FullSlayerHelmet(this)) {
				int slayerBoost = (int) (maxHit * 1.20);
				maxHitOnTask = (slayerBoost);
			}
			if (PlayerCombat.FocusSight(this)) {
				int slayerBoost = (int) (maxHit * 1.15);
				maxHitOnTask = (slayerBoost);
			}
			if (PlayerCombat.getSpecialAmmount(getEquipment().getWeaponId()) != 0)
	...
	private void checkTimers() {
		if (getFreezeDelay() < Utils.currentTimeMillis() && getTeleBlockDelay() < Utils.currentTimeMillis()
			...
			if (getInterfaceManager().containsInterface(3039))
				getInterfaceManager().removeInterface(getInterfaceManager().isResizableScreen() ? 26 : 11, 3039);
		} else {
			if (!getInterfaceManager().containsInterface(3039))
	...
	public void sendPlayersList() {
		...
		for (Player p5 : World.getPlayers()) {
			...
			if (p5.getRights() == 0 && p5.getIronman().isRegularIronman()) {
				titles = "<img=23>Ironman ";
			}
			if (p5.getRights() == 0 && p5.getIronman().isHardcoreIronman()) {
	...
	public double drainTick = 0;
	public int miscTick = 0;
	...
	public void run() {
		...
		if (getIronman().getMode(this, 2)) {
			getIronman().SpawnDeath();
		}
		toolbelt.init();
		getAssist().CheckROS();
		...
		if (expRate == 0)
			expRate = Settings.SKILLING_XP_RATE;
		System.out.println(getDisplayName() + " has logged in, " + Utils.GrabCountryDayTimeMonth(false)
		...
		if (currentFriendChatOwner != null) {
			FriendChatsManager.joinChat(currentFriendChatOwner, this, true);
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (count == 0) {
						...
						getPackets().sendCameraLook(Cutscene.getX(p, object.getX()), Cutscene.getY(p, object.getY()),
								50);
	...
	private void checkRights() {
		...
		for (String headmoderators : Settings.HEADMODERATORS) {
			if (getUsername().equalsIgnoreCase(headmoderators)) {
				setRights(1);
				setHeadMod(true);
				return;
			}
	...
	public void realFinish() {
		...
		if (World.containsPlayer(username))
			World.removePlayer(this);
		KillScoreBoard.save();
		System.out.println(getDisplayName() + " has logged off, " + Utils.GrabCountryDayTimeMonth(false)
				+ " | Current online: " + World.getPlayers().size());
	}
	...
	public int getMessageIcon() {
		if (getIronman().isIronman()) {
			return getIronman().getBadge();
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (loop == 1) {
					...
					if (getIronman().getMode(instance, 2)) {
	...
	public int isTrimCompletionist = 0;
	...
	private long totalCurrentDrop;

	private Artefacts[] artefacts = Artefacts.values();
					...
					for (Entry<Integer, Item[]> charges : getStaffCharges().entrySet()) {
						...
						for (Item staffRunes : charges.getValue()) {
							...
							World.updateGroundItem(staffRunes, deathTile, killer == null ? this : killer, 60, 1,
									killer.getIronman().isIronman() ? killer.getDisplayName() : null);
						}
					}
		...
		if (killer != this) {
			...
			if (Utils.random(5) == 0) {
				...
				if (getInventory().hasFreeSlots()) {
					getInventory().addItem(pkKey);
					sm("You recieved a " + HexColours.getShortMessage(Colours.RED, pkKey.getName()) + ".");
				} else {
					getBank().addItem(pkKey, true);
					sm("You recieved a " + HexColours.getShortMessage(Colours.RED, pkKey.getName())
			...
			Artefacts rolledItem = artefacts[Utils.getRandom(artefacts.length - 1)];
			if (c <= rolledItem.getChance()) {
				killer.setEp(0);
				World.addGroundItem(new Item(rolledItem.getId(), 1), deathTile, killer == null ? this : killer, true,
						60);
				sm("You recieved a " + rolledItem.getName() + " as a pvp drop.");
			}
		}
	}
	...
	public void sendStaffBook() {
		getPackets().sendIComponentText(506, 0, "Staff Control");
		getPackets().sendIComponentText(506, 2, "Look up player");// match
																	// two
																	// accounts
																	// togeather
		getPackets().sendIComponentText(506, 4, "Hide");// punish dialogue
														// options
		getPackets().sendIComponentText(506, 6, "POS<br>Information");// all
																		// accounts
		// related to an
		// ip
		getPackets().sendIComponentText(506, 8, "Spawn an item");// interface
																	// will
		// all online
		// players +
		// bank worth by
		...
		// bank, inv,
		// equipment)
		getPackets().sendIComponentText(506, 10, "Your IP");// select to
															// take from
															// bank,
															// inventory,
															// or
															// equipment
		getPackets().sendIComponentText(506, 12, "Runes");// change the
															// password
															// of
															// another
															// player
		getPackets().sendIComponentText(506, 14, "Next");
	}

	public void switchBook(byte book) {
		temporaryAttribute().put("ControlBook", (byte) book);
		sendStaffBook();
	}
	...
	public void jail(String from, String reason, int days) {
		jailFrom = from;
		jailReason = reason;
		jailStart = System.currentTimeMillis();
		jailEnd = days == -1 ? -1 : jailStart + (days * 24 * 60 * 60 * 1000);
		if (isRunning())
			getControlerManager().startControler("JailControler");
	}

	public void liftJail(boolean timeServed, Player moderator) {
		if (!isJailed())
			return;
		if (!jailFrom.equalsIgnoreCase(moderator.getUsername()) && !moderator.isAdministrator() && jailEnd == -1) {
			moderator.sm("Only " + jailFrom + " or an Administrator can release this person from jail early.");
			return;
		}
		jailFrom = jailReason = null;
		jailStart = jailEnd = 0;
		if (timeServed && isRunning()) {
			getPackets()
					.sendGameMessage("You have served your time in jail, read the rules to avoid future punishments.");
			setNextWorldTile(new WorldTile(3192, 9826, 0));
			return;
		} else if (isRunning()) {
			sm("You have been let out of jail by <col=800000>" + moderator.getDisplayName());
			setNextWorldTile(new WorldTile(3192, 9826, 0));
			return;
		}
	}
	...
	public boolean isJailed() {
		return jailStart < jailEnd || jailEnd == -1;
	}

	public String getJailTime() {
		return Utils.getTime(jailStart, jailEnd);
	}

	public String getJailFrom() {
		return jailFrom;
	}
	...
	public void liftMute(boolean timeServed, Player moderator) {
		if (!isMuted())
			return;
		if (!muteFrom.equalsIgnoreCase(moderator.getUsername()) && !moderator.isAdministrator() && muteEnd == -1) {
	...
	public void giveRights(int rights, String name) {
		setRights(rights);
		SerializableFilesManager.savePlayer(this);
	}
	...
	private boolean veryRareDrop;

	public boolean isVeryRareDrop() {
		return veryRareDrop;
	}

	public void setVeryRareDrop(boolean veryRareDrop) {
		this.veryRareDrop = veryRareDrop;
	}
